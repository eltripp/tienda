...
export default function CreateAdminPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(error instanceof Error ? error.message : "Error al crear administrador");
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="flex min-h-screen items-center justify-center bg-slate-950 p-4">
      <div className="w-full max-w-md space-y-6">
        <div className="text-center">
          ...
          <h1 className="mt-4 font-heading text-2xl text-slate-100">TiendaNexus Admin</h1>
          <p className="text-sm text-slate-500">Crear cuenta de administrador</p>
        </div>
        ...
        <p className="text-center text-xs text-slate-500">
          © 2023 TiendaNexus. Todos los derechos reservados.
        </p>
      </div>
    </div>
  );
